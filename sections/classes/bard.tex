...
\textbf{Smuggler (Ex):} A bard receives an insight bonus to \skill{Bluff} and \skill{Sleight of Hand} checks equal to \onehalf his bard levels.
...
\textit{Coolheaded:} A bard with this trade secret may take 10 on \skill{Bluff}, \skill{Diplomacy}, and \skill{Perform} checks. A bard must be at least 12th level to select this trade secret.
...
\textit{Poisonbane (Ex):} A bard with this trade secret makes \skill{Craft} (alchemy) checks with advantage.

% \textit{Poison Resistance (Ex):} A bard with this trade secret receives a +4 bonus to saving throws against poisons.

\textit{Potion Seller (Su):} A bard with this trade secret can craft \spell{cure light wounds} potions as alchemical items, using \skill{Craft} (alchemy) (DC 25) and caster level equal to his bard level. These potions are alchemical vials and expire after 1 week. A bard must be at least 8th level to select this trade secret.
...
\textit{Skilled:} A bard with this trade secret adds \onequarter his bard level as a competence bonus to one of the following skills: \skill{Appraise}, \skill{Craft}, \skill{Heal}, \skill{Perform}, \skill{Profession}, \skill{Sense Motive} or \skill{Sleight of Hand}. This trade secret may be chosen more than once, each time it applies to a different skill.
...
\textit{Mind Blank (Sp):} At 18th level, a bard's mind becomes completely sealed against involuntary intrusion as per the \spell{mind blank} spell. This spell-like ability is always considered active.
