\documentclass[twocolumn]{d20}
\usepackage{Nouveaud}
\usepackage{ucs}
\usepackage{multirow}
\usepackage{fdsymbol}
\usepackage{pdfpages}
\usepackage{tikz}
\usepackage{tikzpagenodes}
\usepackage[unicode,colorlinks]{hyperref}
\hypersetup{
  bookmarksnumbered=false,
  bookmarksopen=true,
  bookmarksopenlevel=1,
  allcolors=LinkColor,
  % pdfborderstyle={/S/U/W 1},
  pdfstartview=FitBH,
  pdfpagemode=UseOutlines,
  pdfpagelayout=TwoPageRight,
  pdftitle={Dark Sun d20},
  pdfauthor={Caio Freitas de Oliveira},
  pdfkeywords={d20 System}
}

\title{Dark Sun d20}
\newcommand{\WOTC}{\par\textit{\small{\textcopyright} Wizards of the Coast.}}
\makeatletter
\@newtikzpagenode{chapter area}{%
    \def\southwest{\pgfpoint{0}{\paperheight-\current@textarea@top-7mm}}%
    \def\northeast{\pgfpoint{\paperwidth}{\paperheight}}%
}
\@newtikzpagenode{line area}{%
    \def\southwest{\pgfpoint{0}{\paperheight-\current@textarea@top-7mm-1.5pt}}%
    \def\northeast{\pgfpoint{\paperwidth}{\paperheight-\current@textarea@top-7mm}}%
}
\def\@makechapterhead#1{%
  {
    \centering
    \begin{tikzpicture}[remember picture,overlay]
      \fill[TableColor] (chapter area.north west) rectangle (chapter area.south east);
      \fill[black] (line area.north west) rectangle (line area.south east);
      % \node[rotate=180,anchor=north] at (0,-0.4) { \includegraphics[width=10cm]{images/chapter.png} };
      % \node[anchor=north] at (0,2) { \color{white}\bfseries\LARGE#1 };
    \end{tikzpicture}

    \imfellEnglish\color{ChapterColor}%
    \begin{tabular}{p{25mm}r{\textwidth-25mm}}

    \ifnum \c@secnumdepth >\m@ne%
      \if@mainmatter%
        \Large\contourlength{.6pt}\contournumber{10}\contour{white}{
          \@chapapp\space\thechapter%
        }
      \fi%
    \fi%
    &
    \Huge\bfseries\raggedleft{
      \contourlength{0.1pt}\contournumber{20}\contour{white}{
        \MakeUppercase{#1}
      }
    }
    \end{tabular}
  }%
}
\makeatother
\renewcommand{\LettrineFontHook}{\color{ChapterColor}\Huge\Nouveaudfamily}
\renewcommand\labelitemi{$\smallblackdiamond$}
\newcommand\threequarters{\unichar{"00BE} }
\newcommand\onehalf{\unichar{"00BD} }
\newcommand\onequarter{\unichar{"00BC} }
\begin{document}
\includepdf[pages=-]{credits-and-legal.pdf}

\tableofcontents

\input{chapters/1-introduction.tex}
\input{chapters/2-abilities.tex}
\input{chapters/3-character-races.tex}
\input{chapters/4-character-description.tex}
\input{chapters/5-character-classes.tex}
\input{chapters/6-skills.tex}
\input{chapters/7-feats.tex}
\input{chapters/8-prestige-classes.tex}
\input{chapters/9-equipment.tex}
% magic items 10
% combat 11
\input{chapters/12-magic.tex}
\input{chapters/13-spells.tex}
\input{chapters/14-psionics.tex}
% psionic powers
\input{chapters/17-adventuring.tex}
\input{chapters/18-life-on-athas.tex}
% planes
% traps


\listoftables

{
\clearpage
\includepdf[pages=-]{ogl.pdf}
}
\end{document}
