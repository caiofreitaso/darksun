...
}
{
	...
	The hidden \emph{sigil} cannot be detected by normal observation, and \spell{detect magic} reveals only that the entire text is magical.

	A \spell{dispel magic} can remove the \emph{sigil}. An \spell{erase} spell destroys the entire page of text.

	\emph{Sepia snake sigil} can be cast in combination with other spells that hide or garble text, such as \spell{secret page}.
